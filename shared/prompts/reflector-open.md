...
- Check arc state: `arc list --ready` — what's done, what's open, what's stalled?
...
- **Does the plan still make sense?** Given what we now know, is the arc state still the right shape? Should priorities shift?
...
- Update arc items if priorities have shifted (`arc done`, `arc new`, adjust tactical steps)
