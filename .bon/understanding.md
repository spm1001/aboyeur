...
- **TEST-RUNNER TENSION opened by the ws-drop → owned by aby-pizufo (Phase 4).** node 20.19 (tube) has NO global `WebSocket` (landed unflagged in Node 21+), so with `ws` gone the WebSocket-touching tests HANG under `node --test` — `npm test` times out. They pass under `bun test`. But a naive `test: "bun test"` is WRONG: the DAEMON tests (`context-queue` etc.) use `describe()` inside `test()`, unimplemented in bun's node:test compat (bun #5090), so they fail under bun. **The two categories need different runners — exactly the SPLIT aby-pizufo prescribes** (channel/bridge → `bun test`; daemon → `node --test` on `dist/`). Until Phase 4 lands, run bridge tests with `bun test <files>`, daemon tests with node. Verified 2026-07-15.
