...
2. **Mesh-at-birth** (an interactive session that RECEIVES inbound as `<channel>` tags) — **SHIPPED 2026-07-19 (aby-pafada): `claudem` (plain Teams) or `-m` on any wrapper**, appending `--dangerously-load-development-channels plugin:sonnette@batterie` (the docs-canonical per-entry bypass; rides the installed plugin, any cwd, no mcp-config). One trust dialog per launch. **Teams-billed sessions ONLY — Vertex sessions cannot bind channels at all** ("Channels are not available on third-party providers", measured; the plugin's tools/outbound still work there). A bridge-connected session is NOT automatically inbound-capable — only a flag-born/allowlisted one is.
...
## Sonnette SHIPPED — sonnette@batterie 1.16.0 (2026-07-19, aby-zufefu)

The packaging problem is solved and the plugin is live in the public marketplace. What shipped and how:

- **The vendoring answer: a committed single-file bundle.** The batterie assembler vendors source trees with `node_modules` excluded (mise's `uv sync` solution has no bun equivalent), so aboyeur commits `sonnette/conductor-channel.js` — `bun build src/conductor-channel.ts --target=bun` (218 modules, 0.5MB, MCP SDK inlined; `npm run build:sonnette`). The dev hot path (`.mcp.json` → `bun src/`) is untouched; only publishing rebuilds. **Drift guard:** aboyeur CI job `sonnette-bundle-fresh` rebuilds with pinned bun 1.3.14 and `git diff --exit-code sonnette/` — a src/ edit without a rebuild goes red, not stale-to-clients. **After editing channel code: `npm run build:sonnette` and commit the bundle too.**
- **The bun-on-PATH answer: `sonnette/run.sh`.** plugin.json runs `bash ${CLAUDE_PLUGIN_ROOT}/sonnette/run.sh ${CLAUDE_PLUGIN_ROOT}/sonnette/conductor-channel.js`; the wrapper searches PATH then `~/.bun/bin`, `~/.local/bin`, `/opt/homebrew/bin`, `/usr/local/bin` and fails LOUD on stderr when bun is absent (MCP fails soft — a bare `command:"bun"` ENOENTs silently; the 07-14 landmine). Bundle path passes as an arg so the assembler's entry-point guard existence-checks both files. Fleet: tube ✓; **Mac needs bun installed before sonnette works there** (then `curl -fsSL https://bun.sh/install | bash` suffices — wrapper finds `~/.bun/bin`).
- **Wiring:** batterie `assemble.sh` PLUGINS `sonnette:aboyeur` (+ root-anchored `/dist` exclude — local previews were vendoring working-tree tsc output); batterie marketplace.json entry; batterie-lint `REPO_NAMES` `sonnette→aboyeur` (without it the lint silently SKIPs — fail-open). Suite bumped 1.15.1→1.16.0 via publish.py (2-repo push); assemble run green in 20s; installed + verified on tube same hour.
...
- **Double server in aboyeur's cwd — FIXED in sonnette 1.16.1 (aby-suwawo, 2026-07-19).** With sonnette installed user-scope, a session in THIS repo loads BOTH the project `.mcp.json` `conductor-channel` AND the plugin — two bridges deriving the SAME agent id from `CLAUDE_CODE_SESSION_ID`. Before the fix they entered a **perpetual supersession-recovery rotation** (~10s cadence): each recovery superseded the sibling, whose health check recovered back, because `recoveryAttempted` re-arms on every successful `conductor_connected` (line 447), so aby-tarafo's yield + health-check-revive combined into a war. **Fix:** a shared per-agentId `owner.pid` file (`pid:birthMs`), claimed on each successful connect; at `reconnect()` a process yields PERMANENTLY iff a live, strictly *older* sibling owns the id (pid tiebreak). Discriminator is AGE not liveness — the aby-tarafo mid-session-restart survivor is always the *older* process, so it always revives and never wrongly yields (a liveness-only rule raced and broke tarafo revive; caught pre-ship by `tests/suwawo-two-process.mjs`, a two-process real-mesh test). The health check stops polling once permanently yielded; owner released on clean shutdown. If a same-id war ever recurs (e.g. `recoveryAttempted` logic changes), that test is the repro. Three victim surfaces: (a) BOTH tool-visible servers report "NOT delivered: not connected to mesh" while a shadow copy holds the socket (nowabu surfacing the truth); (b) **every OTHER mesh-at-birth session's screen fills with `← sonnette: Peer online: <id>` tags at 10s cadence** — the flap is estate-wide noise, aby-paluvu's cousin with a different mechanism; (c) the shared bridge dir's status file is written by racing processes (trustworthy for neither). Recovery: kill the duplicate BY PID (`ps -o pid,ppid` to map servers→CC session first — a same-cwd process may belong to ANOTHER live session; confirmed 2026-07-19 that a same-cwd `bun` server belonged to a live Fable `/open` session, NOT an orphan); the survivor reconnects cleanly (verified: post-kill, one server, clean PING/PONG, zero supersessions for 40s). **NOT an aby-tarafo regression** — aby-tarafo holds for the one-server case (a concurrent single-server session was observed flap-free the same moment this one flapped). This is a NEW topology, born the night sonnette was installed (2026-07-19 ~22:46), confined to aboyeur's OWN cwd — the only place with a project `.mcp.json` conductor server AS WELL AS the global plugin. Real sonnette users (plugin only) and all other-cwd sessions run one server and never see it. Mechanism confirmed: aby-tarafo's yield-on-supersede AND its 10s health-check-revive each work alone, but combine into a perpetual ~10s war when a same-id sibling keeps superseding, because `recoveryAttempted` re-arms on each successful reconnect (the "one recovery then stop" guard never latches). Field report: **aby-suwawo** (fix candidates: per-lifetime recovery flag; or a bridge-dir lockfile so a second same-id server yields at startup). Interim: expect it on every aboyeur-cwd reload until fixed; options remain `claude plugin disable sonnette` locally or dropping `.mcp.json`.
...
**Docs cross-check (same night, `code.claude.com/docs/en/channels{,-reference}.md` — Sameer's pointer):** the live doc CONFIRMS the tested schema exactly (top-level `{channelsEnabled, allowedChannelPlugins:[{marketplace,plugin}]}` — so the /etc failure was not a schema miss), confirms `allowedChannelPlugins` REPLACES the Anthropic-curated default list (which is just `claude-plugins-official`'s channel plugins: telegram/discord/imessage — **NB those run dialog-free on this org TODAY, relevant to aby-vecaki**), and documents the dev flag's plugin-ref form as canonical (`--dangerously-load-development-channels plugin:name@marketplace`, per-entry bypass, dialog per launch). The doc says admins set the org allowlist "in managed settings" without acknowledging that on Teams the console is the ONLY managed-settings surface and lacks the field — the promise exists, the surface doesn't. Second dialog-free route noted: an Anthropic-partner-coordinated listing in claude-plugins-official (in-app community submission explicitly does NOT reach the channel allowlist). Pro/Max-without-org would skip org checks entirely post-preview (during preview the Anthropic list still gates).
